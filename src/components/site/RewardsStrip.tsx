...
export function RewardsStrip({
  ...
  total: number;
  /** Show skeleton placeholders while the estimator total is being calculated. */
  loading?: boolean;
  ...
  const unlockedLabels = REWARDS.filter((r) => total >= r.threshold).map((r) => r.label);

  useEffect(() => {
    if (!mountedRef.current) {
      mountedRef.current = true;
      prevUnlockedRef.current = unlockedLabels;
      return;
    }
    const newly = unlockedLabels.filter((label) => !prevUnlockedRef.current.includes(label));
    if (newly.length) {
      setAnnouncement(`Unlocked: ${newly.join(", ")}`);
    }
    prevUnlockedRef.current = unlockedLabels;
  }, [unlockedLabels]);
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <Reveal>
          <div className="bg-white rounded-[2rem] p-6 sm:p-8 ll-card">
            {/* Screen-reader only announcement for badge unlock transitions */}
            <div aria-live="polite" aria-atomic="true" className="sr-only">
              {announcement}
            </div>

            {loading ? (
              <RewardsSkeleton cta={!!cta} />
            ) : (
              <>
...
function RewardsSkeleton({ cta }: { cta: boolean }) {
  return (
    <>
      <div className="mx-auto mb-2 h-3 w-40 rounded-full bg-slate-200 animate-pulse" />
      <div className="mx-auto mb-6 h-3 w-56 rounded-full bg-slate-200 animate-pulse" />

      <div
        className="-mx-6 sm:mx-0 px-6 sm:px-0 overflow-x-auto sm:overflow-visible [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        aria-busy="true"
        aria-label="Calculating rewards"
      >
        <ul className="flex sm:grid sm:grid-cols-4 gap-4 sm:gap-4 min-w-max sm:min-w-0">
          {REWARDS.map((reward) => (
            <li
            ...
            >
              <div className="flex flex-col items-center rounded-2xl p-2">
                <span className="relative block h-20 w-20 rounded-full bg-slate-100 animate-pulse" />
                <span className="mt-4 block h-4 w-20 rounded-full bg-slate-200 animate-pulse" />
                <span className="mt-1 block h-3 w-16 rounded-full bg-slate-200 animate-pulse" />
              </div>
            </li>
          ))}
        </ul>
      ...
      {cta ? (
        <div className="mt-8 text-center">
          <div className="inline-flex h-12 w-44 rounded-full bg-slate-200 animate-pulse" />
        </div>
      ) : null}
    </>
  );
